...
echo [1/9] Rebuild D4-D5 from the four verified official captures...
...
echo.
echo [2/9] Normalize 152-FZ article -^> point hierarchy...
%PYTHON_EXE% scripts\normalize_pdn_d4_d5_article_points.py
if errorlevel 1 goto :fail

echo.
echo [3/9] Enforce fail-closed D4-D5 quality gate...
%PYTHON_EXE% scripts\audit_pdn_d4_d5_structure.py
if errorlevel 1 goto :fail

echo.
echo [4/9] Extract D6-D9 review candidates with exact lineage...
%PYTHON_EXE% scripts\run_pdn_d6_d9.py
if errorlevel 1 goto :fail

echo.
echo [5/9] Enforce fail-closed D6-D9 candidate quality gate...
%PYTHON_EXE% scripts\audit_pdn_d6_d9.py
if errorlevel 1 goto :fail

echo.
echo [6/9] Build D10-D12 relations and conflict/overlap candidates...
%PYTHON_EXE% scripts\run_pdn_d10_d12.py
if errorlevel 1 goto :fail

echo.
echo [7/9] Enforce fail-closed D10-D12 relation quality gate...
%PYTHON_EXE% scripts\audit_pdn_d10_d12.py
if errorlevel 1 goto :fail

echo.
echo [8/9] Build D13 graph and D14 controlled review queue...
%PYTHON_EXE% scripts\run_pdn_d13_review_queue.py
if errorlevel 1 goto :fail

echo.
echo [9/9] Validate D13 graph and D14/D15 stop states...
%PYTHON_EXE% scripts\audit_pdn_d13.py
if errorlevel 1 goto :fail

echo.
echo ============================================================
echo PASS: PDN conveyor reached quality-checked D13.
