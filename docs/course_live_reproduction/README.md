...
The course is treated as one continuous lifecycle. Each lesson extends, challenges or normalizes the same project rather than producing an isolated homework artifact. The implementation is not changed by this documentation pass.
...
Existing root/project documents remain canonical for implemented behavior. This course package adds missing lifecycle evidence, architecture views, decisions and review material.
...
PRODUCT / PRESALE                     01
  ↓
REQUIREMENTS / NFR / ESTIMATE         02
  ↓
POC → MVP → PRODUCTION STRATEGY        03
  ↓
HLD / C4                              04
  ↓
LLD / COMPONENTS / API                05
  ↓
RAG OPTIONS / EVALUATION              06
  ↓
AGENT / MULTI-AGENT HANDOFFS          07
  ↓
ADR / ARCHITECTURE-AS-CODE            08
  ↓
CTO CHALLENGE / HOSTING DECISION      09
  ↓
ARCHITECTURE GOVERNANCE / TECH DEBT   10
  ↓
...
- `docs/PROJECT_EXECUTION_CONTROL.md` — DoR/DoD, WIP, PoC/ADR/change/debt controls;
- `config/model_stage_registry.yaml` — semantic capability/model-stage policy;
- `father_osint/storage.py` — current provenance-preserving DEV storage semantics;
- `father_osint/collectors/telegram.py` — transport-neutral source integration boundary;
- `father_osint/knowledge_factory.py` — staged source→knowledge processing model.
...
- `EXISTING_VERIFIED` — supported by repository evidence;
