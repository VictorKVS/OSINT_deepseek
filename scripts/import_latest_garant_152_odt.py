...
from father_osint.legal_core import is_152_fz_primary_document
...
def main() -> int:
    ...
    for candidate in candidates[:20]:
        ...
        if not is_152_fz_primary_document(text):
            diagnostics.append(f"{candidate.name}: identity_failed_primary_document")
            continue
        ...
        print("identity_markers=PASS_PRIMARY_DOCUMENT")
        return 0
