...
from father_osint.analysis import Analysis
...
def test_pipeline_stops_when_analyst_is_satisfied(tmp_path: Path):
    ...
    assert result.stop_reason == "analyst_satisfied"
    assert result.final_analysis is not None
    assert result.final_analysis.gaps == []
...
def test_pipeline_is_bounded_when_follow_up_never_resolves(tmp_path: Path):
    ...
    result = DevResearchPipeline(agent, max_cycles=2).run(task)

    assert len(result.cycles) == 2
    assert result.stop_reason == "max_cycles_reached"
    assert result.final_analysis is not None
    assert result.final_analysis.follow_up_task is not None
